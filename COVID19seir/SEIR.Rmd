...
Individuals age at a rate ensuring they spend the correct time in their age bracket. In addition the user can specify a background non-epidemic death rate. In that case I also introduce a birth rate to keep the population constant. The death rate is the same across age groups in the dashboard (although in the source code it need not be) so it only serves to introduce a new flow of susceptibles and a possible endemic equilibrium.

The system of differential equations that governs the system is given below:
 
### Equations

\begin{equation}
\dot{S^{i}} = -\beta_{e}\sum_{j \in N}w^{i,j}_e E_1^i \frac{S^i}{n^i} \sigma(t) -\beta_{0}\sum_{j \in N}w^{i,j}_0 I_0^i \frac{S^i}{n^i} \sigma(t)-\beta_{1}\sum_{j \in N}w^{i,j}_1 I_1^i \frac{S^i}{n^i} \sigma(t)-\beta_{2}\sum_{j \in N}w^{i,j}_2 I_2^i \frac{S^i}{n^i} \sigma(t)-\beta_{3}\sum_{j \in N}w^{i,j}_3 I_3^i \frac{S^i}{n^i} \sigma(t) +\alpha^{i-1} S^{i-1} -\alpha^i S^i - \eta_{-}^{i} S + \eta_{+}^{i} \sum_{j \in N}n^i +\phi R^i \\
\end{equation}

\begin{equation}
\dot{E_0^i} = \beta_{e}\sum_{j \in N}w^{i,j}_e E_1^i \frac{S^i}{n^i} \sigma(t) +\beta_{0}\sum_{j \in N}w^{i,j}_0 I_0^i \frac{S^i}{n^i} \sigma(t) + \beta_{1}\sum_{j \in N}w^{i,j}_1 I_1^i \frac{S^i}{n^i} \sigma(t) + \beta_{2}\sum_{j \in N}w^{i,j}_2 I_2^i \frac{S^i}{n^i} \sigma(t) + \beta_{3}\sum_{j \in N}w^{i,j}_3 I_3^i \frac{S^i}{n^i} -\epsilon_0 E_0^i \sigma(t) +\alpha^{i-1} E_0^{i-1} -\alpha^i E_0^i - \eta_{-}^{i} E_0^i \\
\end{equation}
